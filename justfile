...
deploy:
  nix run "github:nix-community/nixos-anywhere" -- --flake .#nixos-flashstor root@172.16.0.2 2>&1 | tee logs/deploy-$(date +%Y%m%d-%H%M%S).txt
...
rebuild:
  nixos-rebuild switch --flake .#nixos-flashstor --target-host root@172.16.0.2 2>&1 | tee logs/rebuild-$(date +%Y%m%d-%H%M%S).txt
